...
import asyncio
from pyngrok import ngrok # for desktop server
...
def match_profile(member, job):
    scores = {
        ...
        'role': 3 if (member.get('Headline', '') or '').lower() == job.get('Role', '').lower() else 2 if job.get('Role', '').lower() in (member.get('Headline', '') or '').lower() else 0,
        'location': 2 if (member.get('CityName', '') or '').lower() == job.get('JobLocation', '').lower() else 1 if (member.get('CityName', '') or '').lower() in job.get('JobLocation', '').lower() else 0
    }
...
async def fetch_data_async_new(member_id):
    try:
        conn = pyodbc.connect(
            'DRIVER={SQL Server};'
        ...
        queries = {
                    "member": """
                        ...
                        FROM [MPOWER_TEST].[dbo].[Member] AS m
...
@app.route('/get_job_maching_data/<int:job_id>', methods=['GET'])
@async_route
async def get_recommended_profiles_job_maching(job_id):
    try:
        recommendations = await fetch_data_async_new(job_id)
...
# def match_jobs():
#     data = request.json
#     member = data['data']['member'][0]
#     job_posts = data['data']['jobpost']
...
#             'match_percentage': match_percentage_scaled,
#             'candidate_name': f"{member['MemberFirstName']} {member['MemberLastName']}",
#             'candidate_id': member['MemberId']
#         })
    
#     return jsonify(sorted(matches, key=lambda x: x['match_percentage'], reverse=True))
...
def match_jobs():
    ...
    for job in job_posts:
        ...
        matches[str(job['Id'])] = match_percentage_scaled
